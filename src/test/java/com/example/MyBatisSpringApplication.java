...
public class MyBatisSpringApplication {
  ...
  /**
   * 在整合Mybatis的时候，我们需要定义一个，MapperScannerConfigurer，指定一个basePackage扫描包实现Mapper的自动扫描， MapperScannerConfigurer 实现
   * BeanDefinitionRegistryPostProcessor一个BeanFactory 后置处理器 ，在Spring启动的过程中会触发 postProcessBeanDefinitionRegistry 方法的执行
   * 该方法会扫描 basePackages 包下的 Mapper, 然后把Mapper封装成一个一个的 MapperFactoryBean, <b> 核心知识点 : 也就是说 Mapper 是通过 MapperFactoryBean
   * 来创建的, 方法是 MapperFactoryBean#getObject() </b>
   * <p>
   * MapperFactoryBean 继承了SqlSessionDaoSupport 其中提供了一个SqlSessionTemplate来管理 SqlSession， 而在 SqlSessionTemplate
   * 中是通过动态代理来创建的SqlSession。也就是说在SqlSessionTemplate中管理了SqlSession的代理类。
   * <p>
