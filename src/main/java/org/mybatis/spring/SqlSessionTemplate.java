...
public class SqlSessionTemplate implements SqlSession, DisposableBean {
  ...
  public SqlSessionTemplate(SqlSessionFactory sqlSessionFactory, ExecutorType executorType,
    ...
    /**
     * 外面是把 SqlSessionTemplate 当 SqlSession 用, SqlSessionTemplate 调用 selectOne, selectList 实际用的是 sqlSessionProxy
     *
     ...
     * 此外，它还管理会话生命周期，包括根据 Spring 事务配置根据需要关闭、提交或回滚会话。 也就是说在Spring整合了Mybatis之后，不是直接使用SqlSession,而是使用SqlSessionTemplate
     * 来管理SqlSession的生命周期，它是线程安全的，并且和事务一起工作的。SqlSession的默认实现DefaultSqlSession 是线程不安全的
     */
    this.sqlSessionProxy = (SqlSession) newProxyInstance(SqlSessionFactory.class.getClassLoader(),
        new Class[] { SqlSession.class },
        // 会执行到 SqlSessionInterceptor 里面
  ...
  private class SqlSessionInterceptor implements InvocationHandler {
    ...
    public Object invoke(Object proxy, Method method, Object[] args) throws Throwable {
      /**
       * 1.创建一个 SqlSession,默认实现是 DefaultSqlSession，底层一样是通过 SqlSessionFactory#openSession
       *
